...
function global:au_GetLatest {
  ...
  $majVersions | Sort-Object -Descending | ForEach-Object {
    try {
      ...
      $url32Bit = "${releases}${majVersionFolder}${versionFolder}zabbix_agent2-${version}-windows-i386-openssl.msi"
      $url64Bit = "${releases}${majVersionFolder}${versionFolder}zabbix_agent2-${version}-windows-amd64-openssl.msi"
