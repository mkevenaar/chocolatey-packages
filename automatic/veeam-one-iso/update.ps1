...
$buildHistory = 'https://www.veeam.com/kb4357'
...
function Get-VeeamReleaseHistoryUrl {
  ...
  foreach ($row in $rows) {
    ...
    if ($rowText -match 'Veeam ONE\s+([0-9]+)\s+Releases') {
  ...
  if (-not $majorRows) {
    throw "Unable to find Veeam ONE $MajorVersion release rows in $buildHistory."
  }
  ...
  if (-not $releaseHistoryUrl) {
    throw "Unable to find a release-history KB article for Veeam ONE $MajorVersion in $buildHistory."
  }
...
function Get-VeeamIsoFilename {
  ...
  $isoPattern = "VeeamONE_$([regex]::Escape($Version))(?:_[0-9]{8})?\.iso"
  ...
  if (-not $isoMatch.Success) {
    throw "Unable to find an ISO filename for Veeam ONE $Version in $releaseHistoryUrl."
  }
