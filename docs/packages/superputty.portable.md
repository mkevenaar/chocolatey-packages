...
<img
  src="https://cdn.jsdelivr.net/gh/mkevenaar/chocolatey-packages@16c9865113a11ca6482f97f0092c3482896afe02/icons/superputty.png"
  alt="Package icon"
  width="32" height="32"/>
...
[choco-docs-version]: <https://img.shields.io/chocolatey/v/superputty.portable.svg?label=SuperPutty+(Portable)>
[choco-docs-downloads]: <https://img.shields.io/chocolatey/dt/superputty.portable.svg>
[choco-docs-package]: <https://community.chocolatey.org/packages/superputty.portable>

## Usage

To install SuperPutty (Portable), run the following command:

```powershell
choco install superputty.portable
```

To upgrade SuperPutty (Portable), run the following command:

```powershell
choco upgrade superputty.portable
```

To uninstall SuperPutty (Portable), run the following command:
...
SuperPutty is a Windows GUI Application that allows the PuTTY SSH Client to be
opened in Tabs. Additionally there is support for SCP to transfer files.

SuperPutty is written in C# and runs on any Windows platform supporting the .NET
Framework 3.5 or newer.

### Features

- Docking user interface allows personalized workspace and managing multiple
  PuTTY sessions easy
