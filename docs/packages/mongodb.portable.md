...
[choco-docs-version]: <https://img.shields.io/chocolatey/v/mongodb.portable.svg?label=MongoDB+(Portable)>
[choco-docs-downloads]: <https://img.shields.io/chocolatey/dt/mongodb.portable.svg>
[choco-docs-package]: <https://community.chocolatey.org/packages/mongodb.portable>

## Usage

To install MongoDB (Portable), run the following command:

```powershell
choco install mongodb.portable
```

To upgrade MongoDB (Portable), run the following command:

```powershell
choco upgrade mongodb.portable
```

To uninstall MongoDB (Portable), run the following command:
...
- `/installDir:` - set the installation directory of MongoDB - defaults to
  "$env:ChocolateyToolsLocation" A new folder "mongodb" will be created in that
  folder
