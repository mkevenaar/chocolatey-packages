...
[choco-docs-version]: <https://img.shields.io/chocolatey/v/cpu-z.install.svg?label=CPU-Z+(Install)>
[choco-docs-downloads]: <https://img.shields.io/chocolatey/dt/cpu-z.install.svg>
[choco-docs-package]: <https://community.chocolatey.org/packages/cpu-z.install>

## Usage

To install CPU-Z (Install), run the following command:

```powershell
choco install cpu-z.install
```

To upgrade CPU-Z (Install), run the following command:

```powershell
choco upgrade cpu-z.install
```

To uninstall CPU-Z (Install), run the following command:
